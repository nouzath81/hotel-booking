...
<div class="card"><h3>Existing Room Types</h3>
<?php foreach($rows as $r): $fid = 'rt-' . (int)$r['id']; ?><form id="<?=h($fid)?>" method="post" enctype="multipart/form-data" style="display:none"></form><?php endforeach; ?>
<div style="overflow-x:auto"><table><tr><th>Photo</th><th>Type</th><th>Total</th><th>Price</th><th>Description & Amenities</th><th>Actions</th></tr>
<?php foreach($rows as $r): $fid = 'rt-' . (int)$r['id']; ?><tr><td><input type="hidden" form="<?=h($fid)?>" name="id" value="<?=h((string)$r['id'])?>"><?php if($r['photo']): ?><img src="uploads/room-types/<?=h($r['photo'])?>" style="width:90px;height:60px;object-fit:cover;border-radius:6px" alt="Room photo"><?php else: ?>No photo<?php endif; ?></td><td><input form="<?=h($fid)?>" name="type_name" value="<?=h($r['type_name'])?>" required></td><td><input form="<?=h($fid)?>" type="number" name="total_rooms" min="0" value="<?=h((string)$r['total_rooms'])?>"></td><td><input form="<?=h($fid)?>" type="number" step="0.01" min="0" name="default_rate" value="<?=h((string)$r['default_rate'])?>"></td><td><textarea form="<?=h($fid)?>" name="description" rows="3" style="min-width:220px"><?=h($r['description']??'')?></textarea><div><label><input form="<?=h($fid)?>" type="checkbox" name="ac" <?=$r['ac']?'checked':''?>> AC</label> <label><input form="<?=h($fid)?>" type="checkbox" name="wifi" <?=$r['wifi']?'checked':''?>> Wi-Fi</label> <label><input form="<?=h($fid)?>" type="checkbox" name="tv" <?=$r['tv']?'checked':''?>> TV</label> <label><input form="<?=h($fid)?>" type="checkbox" name="breakfast" <?=$r['breakfast']?'checked':''?>> Breakfast</label></div></td><td><input form="<?=h($fid)?>" type="file" name="photo" accept=".jpg,.jpeg,.png,.webp"><button form="<?=h($fid)?>" class="btn" name="action" value="update">Update</button><?php if($r['photo']): ?><button form="<?=h($fid)?>" class="btn" name="action" value="delete_photo">Remove Photo</button><?php endif; ?><button form="<?=h($fid)?>" class="btn btn-danger" name="action" value="delete" onclick="return confirm('Delete this room type?')">Delete</button></td></tr><?php endforeach; ?></table></div></div>
